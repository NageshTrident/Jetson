/*                                                                                  
 *                                                                                  
 *      pixcipub.c     External        28 AUGUST 2020             
 *                                                                                  
 *      Copyright (C)  2006-2020  EPIX, Inc.  All rights reserved. 
 *                                                                                  
...
#include <linux/sched/signal.h>
  ...
  MODULE_DESCRIPTION("PIXCI(R) 64 Bit Driver" ". " "3.8.01" " " "[20.08.28.142152]" ". " "Copyright(C) 2013 EPIX, Inc.");
...
static int __init init_pxdrvlnx(void)
{
    ...
    int r, major;
    int nunit = -1;
    printk(KERN_INFO "pixci" ": " "PIXCI(R) 64 Bit Driver" " V" "3.8.01" " " "[20.08.28.142152]" " initialization\n");
    if (PIXCIPARM)
...
void pxdrv_vma_close(struct vm_area_struct *vma)
{
 ...
 struct page *page;
 void *virt;
 virt = wrapped_pxdrv_mmapdope1(NULL, vma->vm_pgoff<<PAGE_SHIFT, NULL, NULL, NULL);
 if (virt == NULL)
     return(NULL);
 ...
 nopage: pxdrv_vma_nopage,
    ...
    {
 void *virt;
 virt = wrapped_pxdrv_mmapdope1(vma->vm_file, vma->vm_pgoff<<PAGE_SHIFT, NULL, NULL, NULL);
 if (virt == NULL)
 ...
 fault: pxdrv_vma_fault,
    ...
    {
 void *virt;
 virt = wrapped_pxdrv_mmapdope1(vmf->vma->vm_file, vmf->vma->vm_pgoff<<PAGE_SHIFT, NULL, NULL, NULL);
 if (virt == NULL)
     return(VM_FAULT_SIGBUS);
...
int pxdrv_mmap(struct file *filep, struct vm_area_struct *vma)
{
     ...
     int r;
     virtadrs = wrapped_pxdrv_mmapdope1(vma->vm_file, vma->vm_pgoff<<PAGE_SHIFT, &dmaadrs, &size, &pcidev);
     if (virtadrs == NULL)
  return(-EIO);
     if (vma->vm_end-vma->vm_start > size)
  return(-EIO);
     vma->vm_pgoff = 0;
     r = dma_mmap_coherent(&pcidev->dev, vma, virtadrs, dmaadrs, vma->vm_end-vma->vm_start);
...
_cfunregparm0 wrapper_dma_alloc_coherent(int nowarn, struct pci_dev *dev, size_t order, dma_addr_t *dma_handlep, int dma32)
{
 ...
 #else
#if KERNEL_VERSION(4,14,98) == LINUX_VERSION_CODE
dma32 = 1;
#endif
...
int _cfunregparm0 wrapper_gettimeofday(time_t *ssecp, long *usecp)
{
...
int _cfunregparm0 wrapper_getnstimeofday(time_t *ssecp, long *nsecp)
{
    #if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,0,0))
 struct timespec t;
 getnstimeofday(&t);
