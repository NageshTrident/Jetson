/**                                                                               
 *                                                                                
 *      pixcipub.c     External        2-MAY-2023               
 *                                                                                
...
#include <linux/timekeeping.h>
  ...
  MODULE_DESCRIPTION("PIXCI(R) 64 Bit Driver" ". " "3.8.01" " " "[23.05.02.134348]" ". " "Copyright(C) 2013 EPIX, Inc.");
 ...
 **/
  ...
  #define PIXCI_TTY_EXTERNAL
    ...
    void (*pxdrv_ttywakeup) (void *atty) = NULL;
  #endif
...
static int __init init_pxdrvlnx(void)
{
    ...
    int r, major;
    int nunit = -1;
    printk(KERN_INFO "pixci" ": " "PIXCI(R) 64 Bit Driver" " V" "3.8.01" " " "[23.05.02.134348]" " initialization\n");
    if (PIXCIPARM)
...
void _cfunregparm0 wrapper_ttywakeup(void *atty)
{
    #if defined(PIXCI_TTY_EXTERNAL)
 void (*wakeup)(void *tty);
 if ((wakeup = pxdrv_ttywakeup))
     (*wakeup)(atty);
    #endif
}
 ...
 ****/
 ...
 if (wakeup)
     pxdrv_ttywakeup = wakeup;
 return(wrapped_ttyinit(nttyp, majorp, insertflipchar, probe, wakeup));
    }
    int pxdrv_ttyopen(void *tty, int index, void **pxttyp, int baud)
    {
 return(wrapped_ttyopen(tty, index, pxttyp, baud));
    }
