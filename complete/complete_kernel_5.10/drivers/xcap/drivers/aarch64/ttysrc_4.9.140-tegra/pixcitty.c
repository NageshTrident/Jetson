/**                                                                               
 *                                                                                
 *      pixcitty.c         External       24-JAN-2023         
 *                                                                                
...
#include <asm/current.h>
  ...
  #endif
  MODULE_VERSION("3.8.01" "-" "23.01.24.115451");
  /**
 ...
 * The default default is 9600; as the only baud
...
static int pxdrvtty_open(struct tty_struct *tty, struct file *filp)
{
 ...
 return(0);
    #else
 r = pxdrv_ttyopen(tty, tty->index+dev->pxunit, &dev->pxtty);
 if (r)
     return(r);
 #if OLDNOPROBE|OLDPROBE
     tty->driver_data = dev->pxtty;
...
{
    #if NEWPROBE|1
 printk(KERN_INFO "%s %s loaded\n", XCDRV_IDTTYN, "3.8.01" " " "[23.01.24.115451]" );
    #else
 printk(KERN_INFO "%s %s loaded\n", XCDRV_IDTTYN, "3.8.01" " " "[23.01.24.115451]" );
    #endif
    #if NEWPROBE
