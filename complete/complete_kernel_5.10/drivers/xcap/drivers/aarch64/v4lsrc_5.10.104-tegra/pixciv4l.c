/**                                                                               
 *                                                                                
 *      pixciv4l.c         External       2-MAY-2023         
 *                                                                                
...
/**
 *
 * This V4L2 driver requires that the underlying PIXCI(R)
 * board level driver has been initialized with a video format.
 * Such as by using the XCAP GUI or the 'pixcinit' utility.
 *
 * Without initialization, this V4L2 driver will not operate.
 * Further, the 'v4l2-compliance' or similar utilities, if run,
 * may report one or more API failures.
 *
 * Upon open, if the video format has not been initialized,
 * a "frame grabber not initialized, run 'pixcinit'" message
 * be logged.
 *
 **/
...
#include <media/videobuf2-vmalloc.h>
  ...
  #endif
  MODULE_VERSION("3.8.01" "-" "23.05.02.134420");
  /**
 ...
 * 0x01 (bit): Support MMAP and DMABUF
 * 0x02 (bit): Support USERPTR
 *
 * Number of streaming buffers?
 * While many V4L2 drivers default to 32 buffers,
 * we must consider memory consumption with high
 * resolution, high bit depth cameras;
...
#define STREAM_DMABUF 0x10
static unsigned stream_mode = STREAM_MEMMAP;
module_param(stream_mode, uint, 0644);
MODULE_PARM_DESC(stream_mode, "select streaming mode(s), bit 0: memmap, bit 4: dmabuf");
 ...
 * retain the field counter within pixel data
 * (but will cause the image/pixel format to be non-standard).
 *
 * 0x02 (bit): Force description of image data to be 8-bit monochrome;
 * does not modify the image data, effectively presents raw bytes
 * with (re)interpretation up to the application.
 **/
 ...
 * 'buffer done' from IRQ lower half; reducing latency albeit
...
/**
 *
 * Rarely (if ever) touched compile time parameters.
 *
 **/
 ...
 * we build a custom fourcc based on our hint code,
 ...
 *
 * Alternately, use our data_mode (DATA_RAWBYTES) to force
 * description of pixel data as 8 bit monochrome, perhaps with
 * sidse effect of increasing the number of 'pixels' per row so
 * as to accomodate the entire row of the image. The fourcc
 * code will be V4L2_PIX_FMT_GREY, interpretation is in the
 * eye of the beholder.
 *
 **/
static u32
pxdrvv4l2_pixelhintToFourCC(u32 pixelhint, u32 multitap, u32 pixiebits, u32 bitpack, u32 framestampsize)
{
 ...
 if (pixiebits == 14) {
     switch (pixelhint) {
       #if defined(V4L2_PIX_FMT_Y14)
  case PXHINTGREY: return(V4L2_PIX_FMT_Y14);
       #endif
       #if defined(V4L2_PIX_FMT_SBGGR14)
  case PXHINTBAYER3: return(V4L2_PIX_FMT_SBGGR14);
  case PXHINTBAYER2: return(V4L2_PIX_FMT_SGBRG14);
  case PXHINTBAYER1: return(V4L2_PIX_FMT_SGRBG14);
  case PXHINTBAYER0: return(V4L2_PIX_FMT_SRGGB14);
       #endif
     }
 }
 if (pixiebits == 16) {
...
pxdrvv4l2_s_fmt_vid_cap(struct file *file, void *priv, struct v4l2_format *fmtp)
{
 ...
 return(-EBUSY);
    pxdrvv4l2_try_fmt_vid_cap(file, priv, fmtp);
    return(0);
}
static int
...
pxdrvv4l2_enum_frameintervals(struct file *file, void *priv, struct v4l2_frmivalenum *fival)
{
 ...
 return(-ENOTINITIALIZED);
    if (fival->index)
 return(-EINVAL);
    if (fival->width != dev->pxdrv_v4l2fmt.imagexdim
...
pxdrvv4l2_g_parm(struct file *file, void *priv, struct v4l2_streamparm *parmp)
{
 ...
 return(-EINVAL);
    ...
    parmp->parm.capture.extendedmode = 0;
    parmp->parm.capture.capturemode = 0;
...
static int
pxdrvv4l2_s_parm(struct file *file, void *priv, struct v4l2_streamparm *parmp)
{
    struct pxdrvv4l2_dev *dev = video_drvdata(file);
    if (!pxdrvv4l2_isinitialized(dev))
 return(-ENOTINITIALIZED);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,5,0) || IS_CENTOS7
static int
...
#else
static int pxdrvv4l2_g_pixelaspect(struct file *file, void *priv, int type, struct v4l2_fract *fractp)
{
    struct pxdrvv4l2_dev *dev = video_drvdata(file);
    if (!pxdrvv4l2_isinitialized(dev))
 return(-ENOTINITIALIZED);
...
pxdrvv4l2_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 if (ctrl->id-CID_CUSTOM_BASE == 0) {
    ...
    r = pxdrv_v4l2control(dev, dev->pxdrv_plp, ctrl->id-CID_CUSTOM_BASE-NLAYERCTRL, PXDAI_II_CTRL_GET, ctrl->val);
    if (r == PXDAI_II_CTRL_ERROR)
 return(-EINVAL);
    ctrl->val = r;
    return(0);
}
...
{
    struct pxdrvv4l2_dev *dev = vb2_get_drv_priv(vb->vb2_queue);
    #if 1 || LINUX_VERSION_CODE < KERNEL_VERSION(4,5,0)
...
static int pxdrvv4l2_buffer_prepare(struct vb2_buffer *vb)
{
    struct pxdrvv4l2_dev *dev = vb2_get_drv_priv(vb->vb2_queue);
    struct pxdrvv4l2_buffer *buf = container_of(vb, struct pxdrvv4l2_buffer, vb);
...
static int
pxdrvv4l2_ioctl_qbuf(struct file *file, void *priv, struct v4l2_buffer *ib)
{
    if (ib != NULL) {
 ib->flags = 0;
 ib->reserved = 0;
 ib->reserved2 = 0;
    ...
    return (vb2_ioctl_qbuf(file, priv, ib));
}
static void
...
pxdrvv4l2_init_controls(struct pxdrvv4l2_dev *dev)
{
 ...
 #error
    ...
    ctrlconfigs.flags = (stream_mode)? V4L2_CTRL_FLAG_INACTIVE: 0;
...
pxdrvv4l2_fh_open(struct file *filep)
{
 ...
 dev->pxdrv_plp = pxdrv_v4l2open(dev, dev->pxdrv_unit, (stream_mode&(STREAM_MEMMAP|STREAM_DMABUF))? stream_buffers: -1);
 if (!dev->pxdrv_plp) {
...
static const struct v4l2_ioctl_ops pxdrvv4l2_sm_fops_ioctl_ops = {
    ...
    .vidioc_qbuf = pxdrvv4l2_ioctl_qbuf,
...
static void
pxdrvv4l2_remove(struct pxdrvv4l2_dev *dev)
{
    int i;
...
pxdrvv4l2_create_instance(int unit, struct pci_dev *pcidev)
{
 ...
 vbq->io_modes = ((stream_mode&STREAM_MEMMAP)? VB2_MMAP: 0)
   | ((stream_mode&STREAM_USERPTR)? VB2_USERPTR: 0)
   | VB2_READ;
 #if LINUX_VERSION_CODE >= KERNEL_VERSION(4,0,0)
     vbq->io_modes |= ((stream_mode&STREAM_DMABUF)? VB2_DMABUF: 0);
 #endif
 vbq->drv_priv = dev;
 ...
 *vdev = pxdrvv4l2_rm_template;
 ...
 r = video_register_device(vdev, VFL_TYPE_VIDEO, video_nr);
    #else
...
void pxdrvv4l2_probe(void *adev, int unit, struct pci_dev *pcidev)
{
    int r = 0;
...
init_pxdrvv4l2(void)
{
    int r = 0;
    int nPixci = 0;
    printk(KERN_INFO "%s %s loaded\n", XCDRV_IDV4LN, "3.8.01" " " "[23.05.02.134420]" );
